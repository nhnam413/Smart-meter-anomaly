"""Kiểm chứng số liệu báo cáo mà không ghi đè dữ liệu hay mô hình."""
...
import numpy as np
from generate_report_figures import extract_features, load_inputs, validate_inputs
from sklearn.metrics import accuracy_score, f1_score, precision_score, recall_score

PROJECT = Path(__file__).resolve().parents[1]


# Tính SHA-256 cho các artefact cần được bảo toàn.
def fingerprints():
    paths = [
        PROJECT / "data/train_hourly.csv",
        PROJECT / "data/demo_stream.csv",
    ...
    return {
        str(p.relative_to(PROJECT)).replace("\\", "/"): hashlib.sha256(
            p.read_bytes()
        ).hexdigest()
        for p in paths
    }
...
# Kiểm chứng và ghi lại số liệu dùng trong báo cáo.
def collect():
    import pandas as pd
    ...
    expected_scaled = (
        features[bundle["features"]].values - bundle["scaler"].center_
    ) / bundle["scaler"].scale_
    np.testing.assert_allclose(
        bundle["scaler"].transform(features[bundle["features"]].values),
        expected_scaled,
        rtol=1e-12,
        atol=1e-12,
    )
    ...
    for name, data, feat, expected in [
        ("train", train, train_features, (182, 120)),
        ("demo", demo, features, (239, 72)),
    ]:
        stamps = data.index.to_series()
        gaps = stamps.diff()
        missing = (
            int((stamps.iloc[-1] - stamps.iloc[0]) / pd.Timedelta(hours=1))
            + 1
            - len(data)
        )
        mismatch = int(
            (
                (stamps - stamps.shift(24)).loc[feat.index] != pd.Timedelta(hours=24)
            ).sum()
        )
        ...
        result["datasets"][name] = {
            ...
            "lag24_not_24h": mismatch,
            "gaps": {
                str(k): float(v.total_seconds() / 3600)
                for k, v in gaps[gaps > pd.Timedelta(hours=1)].items()
            },
        }
    for i, name in enumerate(bundle["features"]):
        q1, q3 = np.percentile(train_features[name], [25, 75])
        result["scaler"][name] = {
            "center": float(bundle["scaler"].center_[i]),
            "scale": float(bundle["scaler"].scale_[i]),
            "q1": float(q1),
            "q3": float(q3),
            "iqr": float(q3 - q1),
        }
    assert (
        result["scaler"]["is_night"]["iqr"] == 0
        and result["scaler"]["is_night"]["scale"] == 1
    )
    result["model"] = {
        "parameters": bundle["model"].get_params(),
        "offset": float(bundle["model"].offset_),
    }
    result["metrics"] = {
        ...
        "injected_before_warmup": int(demo["is_anomaly"].sum()),
        "injected_valid": int(labels.sum()),
        "types": {},
    }
    for name, expected in [
        ("power_surge", (127, 205)),
        ("voltage_drop", (193, 205)),
        ("night_spike", (77, 135)),
    ]:
        mask = (types == name).to_numpy()
        caught, total = int(predictions[mask].sum()), int(mask.sum())
        assert (caught, total) == expected
        result["metrics"]["types"][name] = {
            "caught": caught,
            "total": total,
            "recall": caught / total,
        }
    for name, mask, expected in [
        ...
        result["cases"][name] = {
            "time": str(timestamp),
            "synthetic_type": str(types.iloc[i]),
            "decision": float(scores[i]),
            "power": float(demo.loc[timestamp, "Global_active_power"]),
            "voltage": float(demo.loc[timestamp, "Voltage"]),
