"""Capture the existing UI in Edge with an isolated profile and temporary SQLite.

Requires Microsoft Edge and the websockets package (capture tooling only).
Uses CDP to capture the rendered page; does not reconstruct or paint the UI.
"""
import base64
import json
import os
...
class CDP:
    ...
    def ready(self, mode):
        ...
        while time.monotonic() < deadline:
            error = self.js("document.querySelector('[data-testid=stException]')?.innerText")
            if error:
    ...
    def screenshot(self, path, clip):
        result = self.call('Page.captureScreenshot', format='png', captureBeyondViewport=True,
                           fromSurface=True, clip={**clip, 'scale': 1})
...
''', encoding='utf-8')
        ...
        with log_path.open('w', encoding='utf-8') as log:
            try:
                processes.append(subprocess.Popen([sys.executable, '-B', '-m', 'streamlit', 'run', str(wrapper),
                    '--server.address=127.0.0.1', f'--server.port={app_port}', '--server.headless=true',
                    '--browser.gatherUsageStats=false', '--theme.base=light'], cwd=PROJECT, env=env,
                    stdout=log, stderr=subprocess.STDOUT, creationflags=subprocess.CREATE_NO_WINDOW))
                wait_http(f'http://127.0.0.1:{app_port}/_stcore/health')
                processes.append(subprocess.Popen([str(EDGE), '--headless=new', '--disable-gpu', '--no-first-run',
                    '--no-default-browser-check', '--hide-scrollbars', f'--remote-debugging-port={debug_port}',
                    '--remote-debugging-address=127.0.0.1', f'--user-data-dir={temp / "edge-profile"}',
                    '--window-size=1600,1100', 'about:blank'], stdout=log, stderr=subprocess.STDOUT,
                    creationflags=subprocess.CREATE_NO_WINDOW))
                pages = json.loads(wait_http(f'http://127.0.0.1:{debug_port}/json/list'))
                cdp = CDP(next(p['webSocketDebuggerUrl'] for p in pages if p['type']=='page'))
                cdp.call('Page.enable')
                cdp.call('Emulation.setDeviceMetricsOverride', width=1600, height=1100, deviceScaleFactor=2, mobile=False)
                cdp.call('Emulation.setEmulatedMedia', features=[{'name':'prefers-color-scheme', 'value':'light'}])
                for mode, filename in [('history','Hinh_4.2_GiaoDien_LichSu.png'), ('queue','Hinh_4.3_XuLy_CanhBao.png')]:
                    cdp.call('Page.navigate', url=f'http://127.0.0.1:{app_port}/?report_view={mode}')
                    cdp.ready(mode)
                    if mode == 'history':
                ...
                # Raster previews are only for checking generated vector diagrams.
                for svg in sorted(FIGURES.glob('*.svg')):
                    cdp.call('Page.navigate', url=svg.as_uri())
                    time.sleep(.4)
                    size = cdp.js("({width:document.querySelector('svg').width.baseVal.value,height:document.querySelector('svg').height.baseVal.value})")
