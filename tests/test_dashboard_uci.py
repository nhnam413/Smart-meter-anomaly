...
"""
            ...
            with closing(sqlite3.connect(test_db)) as conn:
                ...
                """, (
                    "LEGACY-ALERT", "2010-01-01T00:00:00", 1.0, 230.0,
                    0.8, "critical", "voltage_drop", "Cảnh báo từ phiên cũ",
                    "2026-01-01T00:00:00",
                ))
                conn.commit()
            ...
            self.assertTrue(all(
                event["alert_id"].startswith(f"ALT-{run_id}-") for event in events
            ))
            ...
            self.assertEqual(len(app.dataframe), 0)
            with closing(sqlite3.connect(test_db)) as conn:
                self.assertEqual(conn.execute("SELECT COUNT(*) FROM alerts").fetchone()[0], stored_count)
            del app
            gc.collect()
